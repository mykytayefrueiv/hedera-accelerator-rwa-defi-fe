...
const readSymbol = (tokenAddress: `0x${string}`) => readContract({
	abi: tokenAbi,
	functionName: "symbol",
		...
		if (sliceAddress) {
			readSliceAllocation(sliceAddress).then((data) => {
				const allocationTokenNames = Promise.all(
					data.filter((allocationLog: any) => allocationLog.length > 0).map((allocationLog: any) => readSymbol(allocationLog?.[0]?.[0]))
				)
				
				allocationTokenNames.then(tokenNames => {
					setSliceAllocations(
						data.filter((allocationLog: any) => allocationLog.length > 0).map((allocationLog: any, index: number) => ({
