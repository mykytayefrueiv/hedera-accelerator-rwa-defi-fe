...
import { useWriteContract, useReadContract, useWatchTransactionReceipt } from "@buidlerlabs/hashgraph-react-wallets";
import { useState } from "react";
import { hederaTestnet } from "viem/chains";
import { ContractId } from "@hashgraph/sdk";
import { SwapTokenPriceRequestBody, SwapTokenAddLiquidityRequestBody, SwapTokenSwapRequestBody } from "@/types/erc3643/types";
...
export const useOneSidedExchangeSwaps = () => {
    ...
    const checkBalanceOfLiquidityToken = async (token: `0x${string}`): Promise<bigint> => {
        const balance = await readContract({
            ...
            args: [oneSidedExchangeAddress],
        });

        return balance as bigint;
    };

    const estimateTokensSwapSpendings = async (tokenA: `0x${string}`, tokenB: `0x${string}`, amount: bigint) => {
        const [tokenAAmount, tokenBAmount] = await readContract({
            address: oneSidedExchangeAddress,
    ...
    const handleAddTokenLiquidity = (body: SwapTokenAddLiquidityRequestBody) => {
        ...
        return new Promise((res, rej) => {
            writeContract({
                ...
                args: [oneSidedExchangeAddress, body.amount],
            }).then(approveTx => {
