...
import { SwapLiquidityPair } from "@/types/erc3643/types";
import React, { useState, useEffect } from "react";
import { toast } from "react-hot-toast";
import { ethers } from "ethers";
import Select, { SingleValue } from "react-select";
import { useLiquiditySwaps } from "@/hooks/useLiquiditySwaps";
...
export default function TradeFormUniswapPool({ buildingAddress }: Props) {
    const { liquidityPairs, handleSwapTokens } = useLiquiditySwaps(buildingAddress);
    ...
    const handleSwapSubmit = async (e: React.FormEvent) => {
        ...
        if (!amount || !tradeFormData.tokenA || !tradeFormData.tokenB) {
            ...
            try {
                const tx = await handleSwapTokens({
