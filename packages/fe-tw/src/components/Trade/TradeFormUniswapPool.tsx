...
export default function TradeFormUniswapPool({ buildingAddress }: Props) {
    ...
    if (liquidityPairs?.length) {
      ...
      setLiquidityPairsTo(
        liquidityPairs.filter(
          (pair) => pair.tokenA === liquidityPairs[0].tokenA,
        ),
      );
    }
  }, [liquidityPairs]);
  ...
  const handleSelectPairFrom = (
    ...
    setLiquidityPairsTo(
      liquidityPairs.filter(
        (pair) => pair.tokenA === (tokenA?.value as string),
      ),
    );
  };
  ...
  const handleSwapSubmit = async (e: React.FormEvent) => {
    ...
    if (!amount || !tradeFormData.tokenA || !tradeFormData.tokenB) {
      ...
      try {
        ...
        toast.error(
          `Failed to sell tokens: ${(err as { message: string })?.message}`,
        );
