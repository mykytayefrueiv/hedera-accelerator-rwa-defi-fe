...
export const DeployBuildingERC3643TokenForm = ({ onGetLiquidityView, onGetDeployBuildingView }: Props) => {
    ...
    const { createBuildingERC3643Token } = useBuildingAdmin(selectedBuildingAddress as `0x${string}`);
            ...
            <Form className="p-5 space-y-4 p-8 border border-gray-300">
                ...
                <div className="flex gap-5 mt-5">
                    ...
                    <Button
                        ...
                        type="button"
                        color="secondary"
                        onClick={() => onGetLiquidityView(selectedBuildingAddress as `0x${string}`)}
                ...
                {(selectedBuildingAddress && deployedBuildingTokens?.length > 0) && (
                    <p className="text-md text-purple-500 mt-5">You can skip token creation and move on to add liquidity.</p>
                )}
