import { CreateSliceRequestData } from "@/types/erc3643/types";
...
export const INITIAL_VALUES: CreateSliceRequestData = {
   slice: deploySliceFormInitialValues,
   sliceAllocation: addAllocationFormInitialValues,
};

const validateAssetsField = (val: Yup.AnyObject) =>
   val.when(
      "tokenAssetAmounts",
      (
         [assetsAmounts]: [
         ...
         return schema
            .test("token_assets_min", "Minimum count of assets is is 2", (value: string[]) => {
               return value?.some((asset) => !!asset)
                  ? value?.filter((asset) => !!asset).length >= 2
                  : true;
            })
            .test(
               "token_assets_max",
               "Maximum count of assets is 5",
               (value: string[]) => value?.length < 5,
            )
            .test(
               "token_assets_allocation_amount",
               "Every token asset should have allocation assigned",
               (value: string[]) => {
                  return value?.some((asset) => !!asset)
                     ? value.every((val) => (val ? !!assetsAmounts[val] : true))
                     : true;
               },
            )
            .test(
               "token_assets_allocation_amount_100%",
               "Total token assets allocation should be equal to 100%",
               (value: string[]) =>
                  value?.some((asset) => !!asset)
...
export const validationSchema = Yup.object({
   slice: Yup.object().shape({
      ...
      endDate: Yup.string().when(
         "tokenAssets",
         ([_tokenAssets]: string[][], schema: Yup.Schema) => {
