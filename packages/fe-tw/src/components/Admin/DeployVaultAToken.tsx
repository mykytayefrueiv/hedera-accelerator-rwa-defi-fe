...
import { BuildingToken, DeployAutoCompounderRequest, DeployVaultRequest } from "@/types/erc3643/types";
...
import { useCompoundersAndVaultsData } from "@/hooks/useCompoundersAndVaultsData";
...
const DeployVaultForm = ({ handleDeploy }: { handleDeploy: (data: DeployVaultRequest) => void }) => {
            ...
            >
                {({ setFieldValue, values }) => (
                    <Form className="space-y-4">
                        <div>
                            <label className="block text-md font-semibold text-purple-400">Staking token</label>
                            <Select
                                styles={colourStyles}
                                className="mt-2"
                                placeholder="e.g. 0x"
                                ...
                                value={
                                    values.stakingToken
                                        ? {
                                            ...
                                            label:
                                                buildingAssetsOptions.find((t) => t.value === values.stakingToken)
                        ...
                        <div>
                            <label className="block text-md font-semibold text-purple-400">Share token name</label>
                            <Field
                                name="shareTokenName"
                        ...
                        <div>
                            <label className="block text-md font-semibold text-purple-400">Share token symbol</label>
                            <Field
                        ...
                        <div>
                            <label className="block text-md font-semibold text-purple-400">Fee receiver address</label>
                            <Field
                                name="feeReceiver"
                                className="input input-bordered w-full mt-2"
                                placeholder="e.g. 0x"
                        ...
                        <div>
                            <label className="block text-md font-semibold text-purple-400">Fee token</label>
                            <Field
                                name="feeToken"
                                className="input input-bordered w-full mt-2"
                                placeholder="e.g. 0x"
                        ...
                        <div>
                            <label className="block text-md font-semibold text-purple-400">Fee percentage</label>
                            <Field
                                name="feePercentage"
                                className="input input-bordered w-full mt-2"
                                placeholder="e.g. 20"
...
const DeployAutoCompounderForm = ({ handleDeploy }: { handleDeploy: (data: DeployAutoCompounderRequest) => void }) => {
    const initialValues = {
        tokenName: '',
        tokenSymbol: '',
        tokenAsset: '',
    };
    // const { vaults } = useCompoundersAndVaultsData();

    const assetSelectOptions = [0, 1].map(vault => ({
        value: '0x3F82b8cdDcf1162c3308f76391E2D8A7037F54e9',
        label: 'Asset',
    }));
            ...
            >
                {({ setFieldValue, values }) => (
                    <Form className="space-y-4">
                        ...
                        <div>
                            <label className="block text-md font-semibold text-purple-400">Asset token</label>
                            <Select
                                styles={colourStyles}
                                className="mt-2"
                                placeholder="e.g. 0x"
...
export const DeployVaultAToken = () => {
    ...
    return (
        <div>
            {deployStep === 1 ?
                ...
                <DeployVaultForm handleDeploy={onSubmitDeployStep} />
            }
            {txResults.autoCompounder && <div className="flex mt-5">
                <p className="text-sm font-bold text-purple-600">
                    Deployed Auto Compounder Tx Hash: {txResults.autoCompounder}
                </p>
            </div>}
            {txResults.vault && <div className="flex mt-5">
                <p className="text-sm font-bold text-purple-600">
