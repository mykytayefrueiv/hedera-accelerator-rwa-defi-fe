...
export async function sellBuildingTokenForUSDC(
...
export async function buyBuildingTokenWithUSDC(
  buildingId: `0x${string}`,
  usdcAmount: number,
): Promise<void> {
  await new Promise((resolve) => setTimeout(resolve, 500)); // simulate delay
  ...
  const currentBalance = tradeState.buildingTokenBalances[buildingId] || 0;
  tradeState.buildingTokenBalances[buildingId] =
    currentBalance + tokensToReceive;
}
